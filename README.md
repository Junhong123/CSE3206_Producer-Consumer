...
### Overview
**Producer-Consumer Problem**을 직접 해결해보며 느낀점과 평가를 내린다.
### 개발 환경
Ubuntu 24.04.1 LTS  
gcc 13.3.0  
WSL 2.6.3.0  
### Problem
---
**Producer-Consumer Problem**은 다수의 Thread가 하나의 공유 자원을 동시에 접근하려고 할 때 생기는 문제이다.  
이 과제에서는 **Race Condition** 코드를 활용한다.
```c
#include <pthread.h>
#include <stdio.h>
#define ITER 1000000
...
void *thread_increment(void *arg)
{
    ...
    for (i = 0; i < ITER; i++)
    {
        pthread_mutex_lock(&m);            // 상호배제 락
        while (x == 30)
            pthread_cond_wait(&empty, &m); // CV wait
        val = x;
        printf("%u: %d\n", (unsigned int)pthread_self(), val);
        x = val + 1;
        pthread_cond_signal(&fill);        // decrement 스레드 깨우기
        pthread_mutex_unlock(&m);          // 상호배제 언락
    }
    return NULL;
}
void *thread_decrement(void *arg)
{
    ...
    for (i = 0; i < ITER; i++)
    {
        pthread_mutex_lock(&m);           // 상호배제 락
        while (x == 0)
            pthread_cond_wait(&fill, &m); // CV wait
        val = x;
        printf("%u: %d\n", (unsigned int)pthread_self(), val);
        x = val - 1;
        pthread_cond_signal(&empty);      // increment 스레드 깨우기
        pthread_mutex_unlock(&m);         // 상호배제 언락
    }
    return NULL;
}
...
void *thread_increment(void *arg)
{
    ...
    for (i = 0; i < ITER; i++)
    {
        sem_wait(&empty);  // x가 줄어들 때까지 wait
        sem_wait(&m);      // 상호배제 락
        val = x;
        printf("%u: %d\n", (unsigned int)pthread_self(), val);
        x = val + 1;
        sem_post(&m);      // 상호배제 언락
        sem_post(&fill);   // x가 상승했다고 signal
    }
    return NULL;
}
void *thread_decrement(void *arg)
{
    ...
    for (i = 0; i < ITER; i++)
    {
        sem_wait(&fill);   // x가 상승할 때까지 wait
        sem_wait(&m);      // 상호배제 락
        val = x;
        printf("%u: %d\n", (unsigned int)pthread_self(), val);
        x = val - 1;
        sem_post(&m);      // 상호배제 언락
        sem_post(&empty);  // x가 줄었다고 signal
    }
    return NULL;
}
```
...
| PC | PC_CV | PC_S |
| :-: | :-: | :-: |
| ![PC](./img/PC.png) | ![PC_CV](./img/PC_CV.png) | ![PC_S](./img/PC_S.png) |

두 코드 모두 실행시켜보니 출력되는 x값이 양수만 나오면서 최대 30을 넘지 않는 것을 확인할 수 있다.  
재밌는 점은 아무 처리도 안한 코드의 결과가 1000000이라는 것이다.  
1000000번 ++를하고 1000000번 --를 했는데 결과가 항상 1000000이거나 -1000000이었다.  
...
Result부분에서 확인 했었던 결과가 항상 1000000이거나 -1000000이 나왔던 것은  
printf를 없앴던 버전에서는 결과가 의도한 대로 랜덤하게 나왔던 것으로 보아 printf가 원인인 것 같다.